...
internal static class Program
{
    ...
    /// <summary>
    /// Decrypts a specified csv file, which was created by Xtract Universal with column encryption enabled.
    /// </summary>
    ...
    private static async Task Main(string[] args)
    {
        ...
        using CsvProcessor csvProcessor = new(await reader.ReadToEndAsync());

        string keyXml = await File.ReadAllTextAsync(keyFile);
        using RSACryptoServiceProvider privateKey = new();
    ...
    private static bool GetInfoFromArgs(string[] args)
    {
        if (args[0] == "-h" || args[0] == "--help")
        {
            // help requested.
            Console.WriteLine(
                "This application provides a sample implementation to decrypt data which was encrypted via XtractUniversal using the Column Encryption feature.");
